...
const ScrollFade: React.FC<ScrollFadeProps> = ({
  ...
  const updateScrollState = useCallback(() => {
    ...
    if (fadeMode === 'scroll') {
      ...
      wrapperRef.current?.style.setProperty('--start-fade', `${start}px`)
      wrapperRef.current?.style.setProperty('--end-fade', `${end}px`)
    } else {
      ...
      wrapperRef.current?.style.setProperty(
        '--start-fade',
        `${alwaysShowFade ? fadeSize : canStart ? fadeSize : 0}px`,
      )
      wrapperRef.current?.style.setProperty(
        '--end-fade',
        `${alwaysShowFade ? fadeSize : canEnd ? fadeSize : 0}px`,
      )
    }
      ...
      style={{
        ...
        transition:
          animateFade && fadeMode === 'toggle'
            ...
            : undefined,
        // Dynamically updated via DOM ref for 60fps performance
      }}
    >
      {children}
    </div>
